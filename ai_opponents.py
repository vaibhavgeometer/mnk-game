...
# Format: (m, n, k): [list of opening moves]
OPENING_BOOK = {
...
class TranspositionTable:
    """
    Stores board states to avoid re-calculating the same positions.
    """
    def __init__(self):
        self.table = {}
    ...
    def store(self, z_hash, depth, score, flag, best_move):
        """Stores a result in the transposition table."""
        self.table[z_hash] = (depth, score, flag, best_move)
    
    def lookup(self, z_hash, depth, alpha, beta):
        """
        Retrieves a result from the table if available and applicable.
        """
...
class AI:
    ...
    def level_2_greedy(self, board, player):
        """Greedy strategy: Win immediately, block immediately, or random center."""
    ...
    def alphabeta(self, board, depth, player, alpha, beta, maximizing):
        ...
        # 1. Time Check (check every 128 nodes)
    ...
    def update_heuristics(self, depth, move, delta):
        """Updates killer moves and history heuristic on cutoff."""
        # Killer moves
    ...
    def evaluate(self, board, player):
        """
        Evaluates the current board state from the perspective of 'player'.
        """
    ...
    def evaluate_line(self, line, k, player, opponent):
        """
        Scores a single line (row/col/diag) based on patterns.
        ...
        for i in range(length - k + 1):
            ...
            # If mixed (both X and O), it's blocked.
    ...
    def get_pattern_score(self, count, k, opens, is_player):
        """Returns the score for a specific pattern of stones."""
        s = 0
        ...
        if count == k - 1:
            if opens == 2: s = 5000000 # Live 4
            elif opens == 1: s = 100000 # Dead 4
            
        elif count == k - 2:
            if opens == 2: s = 50000 # Live 3
        ...
        # Defense Weighing: Prefer blocking over attacking if threats are equal
        if not is_player:
            s = int(s * 1.5)
